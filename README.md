...
Within the NodeJS system, there are several high-level components in play. Firstly, the __NodeJS API__ is the toolkit that the client directly installs and accesses from within their __Web Server Application__. This toolkit comes with built-in and additionally importable __Modules__. Now from the Web Server Application, after the user has integrated the backend code into their application, and their frontend is ready to input/output data from the application by using the capabilities of NodeJS, the communication stream begins by collecting into an __Event Queue__. Here, the user and application events from the frontend, such as clicking to access a piece of stored information, are collected in a synchronous manner so that the information can stay processing in realtime. Each event feeds into the __I/O Operating System Interface__, from where it processes through either the __Event Loop__ or the __Thread Pool__. The __Event Loop__ accesses the __V8 JavaScript Engine__ in a non-blocking manner, meaning that the events from the frontend are non-complex, such as a user trying to access or add to a piece of existing data that is set up as the web server application’s backend. On the other hand, the __Thread Pool__ accesses the V8 JavaScript Engine in a blocking manner, meaning that something is needing to access the Web Server Application in a complex manner that is more costly, and cannot efficiently, single-threadedly, and asynchronously access the web server application– such as rendering the HTML of a page, which obviously needs to be done first with more cost. The __V8 Javascript Engine__ itself, is just a translator of the JavaScript into machine code, that communicates to the computer, however the differentiation between the blocking and non-blocking processes is important because it’s how the application is running– meaning that each of these different processes need to feed into the computer in a specific way to ensure the app is functioning exactly how the client expects it to be, hence the V8 JavaScript Engine just accommodates the different levels of communication. Each of these bits of communication is ordered into the collection of __Requests__ that target the Web Server Application, which then follow through on the order of events or errors that are occurring within the application.
